...
@Repository
interface TadoOutsideRepository: JpaRepository<TadoOutsideEntity, LocalDateTime> {

    @Modifying
    @Query(value = "DELETE from tado_outside where time < :beforeTime", nativeQuery = true)
    fun deleteDataBeforeTime(@Param("beforeTime")beforeTime: LocalDateTime)

    @Query(value = "SELECT count(*) from tado_outside where time < :beforeTime", nativeQuery = true)
